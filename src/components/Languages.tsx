...
interface LanguagesProps {
  setLanguages: (data: ILang[]) => void,
  languages: ILang[],
  setIsLanguagesValid: (bool: boolean) => void
}

const Languages = ({ setIsLanguagesValid, setLanguages, languages }: LanguagesProps) => {
  ...
  const handleAddLang = (lang: ILang) => {
    const isLangAdded = languages.find(l => l.code === lang.code)
    if (isLangAdded) {
      const otherLangs = languages.filter(l => l.code !== lang.code)
      setLanguages([...otherLangs, lang])
    } else {
  ...
  const deleteLang = () => {
    if (selectedLang !== null) {
      const otherLangs = languages.filter(l => l.code !== selectedLang.code)
  ...
  }, [languages, setIsLanguagesValid])

  return (
    <div className={`${theme === 'dark' ? 'bg-zinc-800 text-white' : 'bg-white text-zinc-900'} px-2 py-20 relative`}>
      ...
      <div className=''>
        {writingLang.filter(lng => !languages.find(l => l.code === lng.code)).map(lang => (
          <div
            key={lang.code}
